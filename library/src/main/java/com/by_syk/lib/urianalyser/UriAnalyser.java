...
public class UriAnalyser {
    ...
    public static SpannableStringBuilder getRawPathHighlighted(Uri uri, boolean isLightBg) {
        ...
        int[] colorsHighlight = isLightBg ? C.HL_LB : C.HL_DB;
        ...
        ssb.setSpan(new ForegroundColorSpan(colorsHighlight[3]), index1, index2,
                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        if (!TextUtils.isEmpty(scheme)) {
            index2 = scheme.length();
            ssb.setSpan(new ForegroundColorSpan(colorsHighlight[0]), index1, index2,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            index1 = index2 + 3;
        }
        if (!TextUtils.isEmpty(authority)) {
            index2 = index1 + authority.length();
            ssb.setSpan(new ForegroundColorSpan(colorsHighlight[1]), index1, index2,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            index1 = index2;
        }
        if (!TextUtils.isEmpty(path)) {
            index2 = index1 + path.length();
            ssb.setSpan(new ForegroundColorSpan(colorsHighlight[2]), index1, index2,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    ...
    public static String getRealPath(Context context, Uri uri) {
        ...
        } else if ("http".equalsIgnoreCase(SCHEME)) { // HTTP
            result = uri.toString();
        }
    ...
    private static String getDataColumn(Context context, Uri uri, String selection, String[] selectionArgs) {
        ...
        try {
             ...
             *
             * Caused by: android.database.sqlite.SQLiteException: no such column: _data (code 1)
             *
             * SecurityException
             */
            e.printStackTrace();
